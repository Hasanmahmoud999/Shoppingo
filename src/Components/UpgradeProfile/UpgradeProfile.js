...
function UpgradeProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validPhone) {
      setErrMsg(
        <>
          ...
          <p>
            The Phone Number :<br /> consist of 10 number digits <br />
            Must begins with a 09{' '}
          </p>
        </>
      );
      showPopupNote();
    ...
    if (!coords.latitude || !coords.longitude) {
      setErrMsg(
        <>
          ...
          <p>
            Sorry we can't get your location ,bad connection ,please try in
            another time
          </p>
        </>
      );
    ...
    try {
      ...
      setNoteMsg(
        <h4>
          Wait for us to call this number: <br /> {phone} <br /> so we can
        ...
        <Rsection>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Input
              ...
              style={{ marginBottom: '30px' }}
            ></Input>
            <Input2 type="submit">Upgrade</Input2>
          </Form>
        </Rsection>
      </Container>
    </>
